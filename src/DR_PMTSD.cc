...
DR_PMTSD::ProcessHits_constStep( const G4Step* aStep, G4TouchableHistory* )
{
  ...
  /*if (processName == "Cerenkov")
  {
    ...
    if (thePostPVNamesd.contains("ecalDetP_ff"))
    {
      ...
      //CreateTree::Instance()->tot_phot_cer_SDdetected_ff++;
    ...
    else if (thePostPVNamesd.contains("ecalDetP_fr"))
    {
      ...
      hit->SetBounceCount(theTrack->GetCurrentStepNumber());
      fPMTHitsCollection->insert(hit);
      //CreateTree::Instance()->tot_phot_cer_SDdetected_fr++;
    ...
    else if (thePostPVNamesd.contains("ecalDetP_rf"))
    {
      ...
      hit->SetBounceCount(theTrack->GetCurrentStepNumber());
      fPMTHitsCollection->insert(hit);
      //CreateTree::Instance()->tot_phot_cer_SDdetected_rf++;
    }
    else if (thePostPVNamesd.contains("ecalDetP_rr"))
    {
      ...
      hit->SetBounceCount(theTrack->GetCurrentStepNumber());
      fPMTHitsCollection->insert(hit);
      //CreateTree::Instance()->tot_phot_cer_SDdetected_rr++;
    }else{
      ...
      //std::cout<<"origin ------- wierd SD place, Pre: "<<thePrePVNamesd<<" post: "<<thePostPVNamesd<<std::endl;
      return false;
    }
  return true;
  }else{
