...
double sampling_frac_e_scinti = 30;  //56 for circle
bool Count_CherenPhoton_in_scinti_fiber = false;
bool Count_scinti_in_cheren_fiber = false;
bool savepictures = false;
bool calibrate_use_last_pion = false;
...
void get_reso_vs_Ein_dream()
{
    ...
    int npoints = 3; //how many energy points
...
//    aatruemean[4] = 250;
//    aatruemean[5] = 375;
    char dtag[100] = "Square_"; // file tag, shoule be changed
    ...
    {
        sprintf(filenames_p[lp], "%s%s_%gGeV.root", dtag, dpi, aatruemean[lp]);
        sprintf(filenames_e[lp], "%s%s_%gGeV.root", dtag, de, aatruemean[lp]);
...
/*
    ...
    //rotating method
    for (int loop_f = 1; loop_f < npoints; loop_f++)
    {
        ...
        for (Int_t ii = 0; ii < nentries; ii++)
        {
            ...
            xx = (depositedIonEnergyECAL_f[1] + depositedIonEnergyECAL_r[1]) *sampling_frac_e_scinti/ aatruemean[loop_f] / cali_sc[loop_f];
...
*/
    ...
    for (int k = 0; k < npoints; k++)
    {
        ...
        if(calibrate_use_last_pion) {const int last_p = npoints-1;arrmean[k] = rrmeans[k][0] / rrmeans[last_p][1];}
    }
    ...
    if (draw_option == 1)
    {
        for (int k = 0; k < npoints; k++)
        {
            ...
            if(calibrate_use_last_pion) {const int last_p = npoints-1; arrmean[k] = rrmeans[k][1] / rrmeans[last_p][1];}
        }
